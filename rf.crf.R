...
checkCorrAngle <- function (x, mode="median") {
  idx <- which(angle.correction$id == as.character(x[1]))[1]
  if (x[1] %in% angle.correction$id && 
        abs(as.numeric(x[2]) - angle.correction$org_angle[idx]) < 1e-5) {
    if (mode == "mean") {angle.correction$mean_corrected_angle[idx]}
    else {angle.correction$median_corrected_angle[idx]}
  } else { x[2] }
}

corrected.angles <- as.numeric(apply(rawdata[orderid,], 1, checkCorrAngle, "median"))
data <- cbind(corrected.angles, rawdata[orderid,3:5], (rawdata[orderid,7] + rawdata[orderid,8])/2)
...
names(data) <- c("smallestAngle", "ligand1", "ligand2", "bidentate", "avgBL")
