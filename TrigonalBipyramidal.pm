...
sub angleTestStatistic
  ...
  if ($type eq "dev")
    ...
    $chiStat += $distChi;
    my $prob0 = &Statistics::Distributions::chisqrprob(15, $chiStat);

    my $chiStat = 0;
    map {$chiStat += ($$diff[$_] * $$invStds[$_] / 1.1)**2;} (0..(@$diff-1));
    $chiStat += $distChi;
    my $prob1 = &Statistics::Distributions::chisqrprob(15, $chiStat);

    my $chiStat = 0;
    map {$chiStat += ($$diff[$_] * $$invStds[$_] / 1.2)**2;} (0..(@$diff-1));
    $chiStat += $distChi;
    my $prob2 = &Statistics::Distributions::chisqrprob(15, $chiStat);

    my $chiStat = 0;
    map {$chiStat += ($$diff[$_] * $$invStds[$_] / 1.3)**2;} (0..(@$diff-1));
    $chiStat += $distChi;
    my $prob3 = &Statistics::Distributions::chisqrprob(15, $chiStat);

    my $chiStat = 0;
    map {$chiStat += ($$diff[$_] * $$invStds[$_] / 1.4)**2;} (0..(@$diff-1));
    $chiStat += $distChi;
    my $prob4 = &Statistics::Distributions::chisqrprob(15, $chiStat);

    my $chiStat = 0;
    map {$chiStat += ($$diff[$_] * $$invStds[$_] / 1.5)**2;} (0..(@$diff-1));
    $chiStat += $distChi;
    my $prob5 = &Statistics::Distributions::chisqrprob(15, $chiStat);

    my $chiStat = 0;
    map {$chiStat += ($$diff[$_] * $$invStds[$_] / 1.6)**2;} (0..(@$diff-1));
    $chiStat += $distChi;
    my $prob6 = &Statistics::Distributions::chisqrprob(15, $chiStat);

    my $chiStat = 0;
    map {$chiStat += ($$diff[$_] * $$invStds[$_] / 1.7)**2;} (0..(@$diff-1));
    $chiStat += $distChi;
    my $prob7 = &Statistics::Distributions::chisqrprob(15, $chiStat);

    my $chiStat = 0;
    map {$chiStat += ($$diff[$_] * $$invStds[$_] / 1.8)**2;} (0..(@$diff-1));
    $chiStat += $distChi;
    my $prob8 = &Statistics::Distributions::chisqrprob(15, $chiStat);

    my $chiStat = 0;
    map {$chiStat += ($$diff[$_] * $$invStds[$_] / 1.9)**2;} (0..(@$diff-1));
    $chiStat += $distChi;
    my $prob9 = &Statistics::Distributions::chisqrprob(15, $chiStat);

    my $chiStat = 0;
    map {$chiStat += ($$diff[$_] * $$invStds[$_] / 2)**2;} (0..(@$diff-1));
    $chiStat += $distChi;
    my $prob10 = &Statistics::Distributions::chisqrprob(15, $chiStat);

    my $chiStat = $self->covMatChi($diff, $invStds, $invCorrM);
    my $prob = &Statistics::Distributions::chisqrprob(12, $chiStat+ $distChi);
