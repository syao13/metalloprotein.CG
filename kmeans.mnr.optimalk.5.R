...
## normal
normal.nr <- normal.nr[sample(nrow(normal.nr), nrow(normal.nr)/20 * as.numeric(args[4]), replace = FALSE), ]
...
## compressed
compressed.nr <- compressed.nr[sample(nrow(compressed.nr), nrow(compressed.nr)/20 * as.numeric(args[4]), replace = FALSE), ]
...
## combined
all.nr <- all.nr[sample(nrow(all.nr), nrow(all.nr)/20 * as.numeric(args[4]), replace = FALSE), ]
